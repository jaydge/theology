...
APPLIES THE RULE WITHOUT KNOWING ITS PREMISE IS BEFORE JD.**

⚠️⚠️ **JD'S REASONING, DELIVERED WITH HIS `260835-25` RULING ON THE FIVE
`UNCERTAIN` OFFICE ROWS, CUTS AGAINST THIS RULE'S LOAD-BEARING PREMISE.** The
rule declines a pure office on the assumption that it carries **no original
teaching**. ⭐⭐⭐ **JD'S POINT IS THAT COMMENTARY AROUND THE LITURGY IS
ORIGINAL TEACHING EVEN WHEN THERE IS NO HOMILY** — announcements, framing,
explanation of what is being done and why, asides between the parts of the rite.
**If that is right, case 1 declines rows that do contain his own words, and the
rule's central assumption is false rather than merely conservative.**

⛔⛔⛔ **WHAT THIS PASS DID AND DID NOT DO.** It marked the five `UNCERTAIN`
office rows for a future pull batch **with `UNCERTAIN` retained** — a
disposition added to an unchanged status. ⛔ **It did NOT touch the six
`DECLINED-office` rows, did NOT rewrite this rule, and did NOT soften it in
practice.** Whether those six should be revisited is **JD'S RULING** and is
reported to him at `passes/260835-25_five-rulings_close-out.md` §5, together with
the text a softened rule would need. ⭐ **Until he rules, the rule above
governs and a pass meeting this flag should apply the rule as written.**

⭐ **The shape a softened rule would take, recorded so the ruling is cheap to
give:** pull offices rather than declining them; **expect low yield and record it
as low in advance** so a thin batch is not read as failure; and ⛔⛔ **keep
the read-aloud attribution layer non-optional — under JD's reasoning it
matters MORE, not less, because the commentary being sought sits interleaved with
the liturgical text.** ⚠️ **The counter-argument is recorded too: `File 69`
...
⭐⭐⭐ **NEW (260835-37) — RECONCILE `SRC_Channel_Inventory.md` AGAINST
`SRC_Manifest.md` ON VIDEO ID, AND ONLY VIDEO ID. NEVER TRUST A SINGLE
KEY.** Forced by a confirmed defect: `260833-8` keyed the inventory on
video ID; `260834-1` registered five videos keyed on source basename;
nothing ever reconciled the two, and TWO of the five (`File 37`/`File
39`) sat with a live `INCLUDE` verdict in the inventory, already
registered and already mined, until `260835-36` caught them — by
checking only the two videos its own brief happened to name. A full
video-ID sweep at `260835-37` found the OTHER THREE of the same
five-video batch (`File 35`/`File 36`/`File 38`) in the identical state,
still uncaught.

⛔⛔ **THE LESSON IS NOT "CHECK MORE CAREFULLY." IT IS "CHECK ON THE RIGHT
KEY."** Video ID is the only stable identifier this project holds for a
YouTube source: titles are edited by the channel after posting (the
`260835-25` session-renumbering precedent), and basenames are chosen
per-batch by whoever runs the intake and carry no cross-batch discipline.
**A reconciliation keyed on title or basename will drift; one keyed on
video ID cannot, because video ID is assigned once, by the platform, and
never changes.**

✅ **THE STANDING PROCEDURE, going forward, for any pass that registers a
video-sourced File or reclassifies an inventory row:** before minting a
File number for a video already carrying an inventory row, grep
`SRC_Manifest.md` for that row's video ID as a bare 11-character token
(backtick-enclosed or not) across the WHOLE file — not just the batch or
section the pass expects to touch. ⛔ **A hit anywhere is a prior
registration and blocks minting, regardless of what the inventory row's
own decision cell currently says.** `INCLUDE` on a row is never evidence
that a video is unmined. ⭐ **Symmetrically, after any batch registration,
sweep EVERY video ID in that batch against the inventory's decision
cells** — not only the video(s) the delegating brief happens to name — and
correct every stale `INCLUDE`/`DECLINED`/`UNCERTAIN` cell found, as a
dated reclassification retaining the prior verdict verbatim.

⚠️ **Title/basename matching still has a role — it is how `SRC_Manifest.md`'s
own pre-`-meta.json` Files (1-26, no recorded video ID) were reconciled at
`260833-8`'s Step 5b — but it is a WEAKER, SUPPLEMENTARY check, never a
substitute for the video-ID sweep where a video ID exists on both sides.**
A title match that is plausible but not provable (different upload date,
no video ID recorded on one side to cross-check) is FLAGGED for JD, never
silently reclassified — the `260835-30` precedent (a whole batch found
already-registered) is the shape of what a premature title-only match
risks. Full accounting: `passes/260835-37_duplicate-registration-sweep-ls123-owed-items_close-out.md`.
