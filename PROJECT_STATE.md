...
**Last updated: 260728-2** (created 260724-3). Read this file first, before any other project document.
...
|---|---|---|---|
| `PROJECT_STATE.md` | 260728-2 | Backstage | JD + Claude |
| `St_Francis_EMC_Distinctives.md` | 260728-2 | Backstage — findings corpus | JD only |
...
| `validate_project.py` | 260728-2 | ⚠️ **Tooling — REGISTERED 260726-1, STAMPED 260728-1** | JD + Claude |
...
**Next free number by prefix:** `DQ-16` · `IP-13` · **`RV-24`** · **`LS-23`** · **`VP-7`** · **`EXT-3`** · `QA` and `VP` are cross-reference label series, not finding series (see rule 5) · (GV, BP, RC, Rev closed batches)

⚠️ **`EXT` WAS REOPENED 260728-2 (JD's prefix ruling, CL-8).** It had been listed among the closed batches. It is reopened to admit **proven self-authored material on an external platform that no other batch reaches** — `EXT-2` is the YouTube channel masthead bio. ⛔ **`EXT` now holds two different kinds of thing: third-party research (`EXT-1`) and Rev. James's own published words (`EXT-2`). The prefix no longer tells you who is speaking. Read the finding.** *(The wider self-authored corpus, the blogspot archive, does **not** go here — it takes its own prefix `BLOG`.)*
